...
部署完成后，也可以在 `/admin` 的“运行设置”中切换 R2 / S3 兼容 API、修改 S3 配置，以及开关“允许自定义取件码”。后台保存的 S3 Secret 会加密写入 D1；建议额外添加 `STORAGE_CONFIG_KEY` Secret，未添加时会回退使用 `PICKUP_CODE_PEPPER` 派生加密密钥。
...
After deployment, `/admin` **Runtime Settings** can also switch between R2 and S3-compatible storage, update S3 configuration, and toggle custom pickup codes. S3 secrets saved from the admin page are encrypted in D1. Add `STORAGE_CONFIG_KEY` as an extra secret when possible; if omitted, `PICKUP_CODE_PEPPER` is used as the fallback encryption key.
